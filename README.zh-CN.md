...
**网络规则：** 名称不以 `_` 开头的无参数 `public` 方法会暴露给旧式网络调用。为仅限本地或自定义的 `public` 方法添加 `_`，只通过 `[NetworkCallable]` 显式公开所需入口。使用 `NetworkCalling.CallingPlayer` 判断调用者权限，并与接收端所有权分开检查。不要将实例 Master 作为安全或访问控制边界。
